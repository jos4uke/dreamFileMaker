...
def recupEffect(clnEff,listInfo,UNTREATED_CASES_FILE):
	...
	if EFFECT_NAME in list_effect_details_1:	
		...
		if len(geneList_F )< 2:
			...
			#print geneList_details
			#print geneList
			ID_GENE = geneID_formatClean(geneList[0])
			...
			if	(geneList_details[len(geneList_details)-2]) != CODON_DEGENERACY :
					...
					for ID_GENE in geneList:
						...
						RECUP_Effect = geneID_formatClean(ID_GENE) + "\t" + ID_TRANSCRIT + "\t" + EFFECT_NAME + "\t" + NBR_BASE +"\t" + OLD_AA_NEW_AA + "\t" + OLD_CODON_NEW_CODON + "\t" + CODON_NUM_CDS + "\t" + CODON_DEGENERACY
						#print RECUP_Effect   #ok
						return RECUP_Effect
